...
export default function LandingPage() {
  ...
  return (
    <div className="w-full flex-1 pb-12">
      <section id="top" className="panel-card relative mx-auto mt-8 w-full max-w-6xl overflow-hidden rounded-3xl px-6 py-12 md:py-16 lg:py-20">
        <div
        ...
        />
        <div className="pointer-events-none absolute -bottom-28 right-4 h-72 w-72 rounded-full bg-brand-gold/20 blur-3xl" />
        <div className="pointer-events-none absolute -top-20 left-10 h-56 w-56 rounded-full bg-brand-mint/20 blur-3xl float-slow" />

        <div className="relative mx-auto max-w-4xl text-center">
          <p className="subtle-badge reveal-up">Startup Intelligence Platform</p>
          <h1 className="reveal-up-delay mt-5 text-4xl font-bold tracking-tight text-white md:text-6xl">
            Validate startup ideas faster.
            <br />
            <span className="bg-gradient-to-r from-brand-amber via-[#f3deba] to-brand-mist bg-clip-text text-transparent">
              Ship with market confidence.
            </span>
          </h1>

          <p className="mx-auto mt-6 max-w-2xl text-base text-slate-300 md:text-lg">
            Get a focused market report in seconds before you invest weeks in the wrong product.
          </p>
      ...
      <section id="faqs" className="mx-auto mt-16 w-full max-w-6xl px-6 py-12 md:py-16 lg:py-20">
        <div className="mb-10 text-center">
          ...
          <h2 className="text-3xl font-semibold tracking-tight text-white md:text-4xl">FAQs</h2>
        </div>
        ...
        <div className="mx-auto max-w-5xl space-y-3">
          {FAQS.map((item, index) => {
            ...
            return (
              <div
                key={item.question}
                className={`rounded-xl border bg-surface-900/78 shadow-soft transition-all duration-200 ${
                  isOpen ? 'border-brand-gold/45 shadow-glow' : 'border-white/10'
                }`}
              >
                <button
                  type="button"
                ...
                >
                  <div className="min-h-0">
                    <p className="border-t border-white/10 bg-surface-900/45 px-5 py-3 text-sm leading-relaxed text-slate-300">
                      {item.answer}
                    </p>
                  </div>
                </div>
