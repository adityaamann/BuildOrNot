  ...
  return (
    <section id="features" className="mx-auto mt-16 w-full max-w-6xl px-6 py-12 md:py-16 lg:py-20">
      <div className="mb-10 text-center">
        ...
        <h2 className="text-3xl font-semibold tracking-tight text-white md:text-4xl">Designed for fast, high-quality decisions</h2>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {FEATURES.map(({ icon: Icon, title, description }, index) => (
          <article
            key={title}
            className="panel-card relative rounded-2xl p-6 hover:-translate-y-0.5"
          >
            <span className="absolute right-4 top-4 text-[0.65rem] font-semibold tracking-[0.2em] text-slate-500">
              {String(index + 1).padStart(2, '0')}
            </span>

            <div className="mb-4 inline-flex h-11 w-11 items-center justify-center rounded-lg border border-brand-gold/30 bg-brand-gold/12 text-brand-amber">
